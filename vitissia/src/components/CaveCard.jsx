...
export default function CaveCard({ vin, isMobile, onToggleFavori, onDelete, formatRegionName }) {
  ...
  let label = '';
  if (n >= 18) label = 'Exceptionnel';
  else if (n >= 14) label = 'Excellent';
  else if (n >= 10) label = 'Très bon';
  else if (n >= 6) label = 'Bon';
  else if (n >= 2) label = 'Acceptable';
  ...
  const fullStars = Math.floor(count / 4);
  const remainder = count % 4;
  const hasHalfStar = remainder >= 2;
  const totalStars = 5;
    ...
    <div className="flex items-center gap-0.5">
      {Array.from({ length: totalStars }).map((_, i) => {
        if (i < fullStars) {
          ...
          return <i key={i} className="pi pi-star text-gray-500 dark:text-gray-600 text-sm" />;
        }
      })}
    </div>
  );
