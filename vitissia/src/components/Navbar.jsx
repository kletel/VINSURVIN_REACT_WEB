...
const Navbar = () => {
  ...
  // --- RESET localStorage ciblé pour Sommelier ---
  const SOMMELIER_RESET_KEYS = [
    "lastSommelierResult",
    // ajoute ici d’autres clés si le flux Sommelier en utilise :
    // "uuidTable", "vinsFiltre", "repas", "vinChoice", ...
  ];
   ...
   * - reset localStorage si destination === /sommelier
   * - garde la structure de rendu existante (Link)
   */
  const handleNavClick = (e, path) => {
    ...
    if (path === '/sommelier') {
      // reset ciblé avant navigation
      resetSommelierStorage();
    }
    // fermeture de la sidebar si clic depuis la sidebar
    closeSidebar();
  };
