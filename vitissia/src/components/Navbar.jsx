...
const Navbar = () => {
  ...
  const isActivePath = (path) => path === '/' ? (location.pathname === '/' || location.pathname === '/dashboard') : location.pathname === path;
      ...
      <IconContext.Provider value={{ color: "#fff" }}>
          ...
          <div className="relative">
            <div className="backdrop-blur-xl bg-white/70 dark:bg-gray-900/60 border-b border-white/40 dark:border-gray-700 shadow-[0_2px_12px_-2px_rgba(0,0,0,0.15)] px-4 md:px-6 py-2.5 flex items-center gap-4">
              ...
              <div className="hidden md:flex flex-1 items-center gap-1">
                {/* Lien ajouté: Livre de cave */}
                {/*<Link
                  to="/"
                  className={`relative px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${isActivePath('/') ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-600 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400'}`}
                >
                  <span className={`absolute inset-0 rounded-lg ${isActivePath('/') ? 'bg-emerald-500/10 dark:bg-emerald-400/10 ring-1 ring-emerald-500/30 dark:ring-emerald-400/30' : 'hover:bg-gray-900/5 dark:hover:bg-white/5'} backdrop-blur-sm`}></span>
        ...
        {quickLinks.map(link => (
                  <Link
                    key={link.path}
                    to={link.path}
          onClick={(e) => handleProtectedNav(e, link.path)}
              ...
              <div className="flex items-center gap-3 ml-auto md:ml-0">
                ...
                {/* Nom utilisateur desktop (uniquement si connecté) */}
                {isLoggedIn && (
                ...
                >
                  ...
                  {/* Indicateur de statut amélioré (seulement si connecté) */}
                  {isLoggedIn && (
                  ...
                  {section.items.map((item, idx) => {
                    ...
                    return (
                      <li key={idx}>
                        <Link
                          to={item.path}
                          onClick={(e) => handleProtectedNav(e, item.path)}
