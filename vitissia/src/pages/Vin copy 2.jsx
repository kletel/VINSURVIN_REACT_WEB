...
const Vin = () => {
  ...
  const getModifiedFields = () => {
    const modified = {};
    const validTypes = optionTypeVin.map(option => option.value); // Récupérer les types valides

    Object.keys(vin).forEach((key) => {
      if (key === "Type" && !validTypes.includes(vin[key])) {
        modified[key] = ""; // Si le type n'est pas valide, le définir à ""
      } else if (vin[key] !== initialVin[key]) {
        modified[key] = vin[key]; // Ajouter uniquement les champs modifiés
      }
    });
  ...
  const handleMetClick = async (met) => {
    ...
    try {
      ...
      const response = await fetch(`${config.apiBaseUrl}/4DACTION/react_analyseIA_Met`, {
        method: 'POST',
        headers: authHeader(),
        body: formData,
      });
      ...
      if (response.ok) {
        const data = await response.json();
        // Stocker les données de la recette et rediriger
        ...
        toast.current.show({
          severity: 'error',
          summary: 'Erreur',
          detail: 'Impossible de charger la recette',
          life: 3000
        });
      }
    } catch (err) {
  ...
  const getNoteDescription = (value) => {
    ...
    if (value >= 91) return 'Très bon';
    if (value >= 87) return 'Bon';
    if (value >= 84) return 'Acceptable';
                  ...
                  <div className={`mb-4 ${!isEditing ? 'pb-1 border-b ' : ''}`}>
                    ...
                    {isEditing ? (
                      <Select
                        ...
                        defaultValue={optionCouleur.find((option) => option.value === vin?.Couleur) || ""}
                        isClearable={isClearable}
                        isSearchable={isSearchable}
                        name=""
                        placeholder="Couleur"
                        options={optionCouleur}
                        onChange={(selectedOption) => handleInputChange(selectedOption?.label || '', 'Couleur')}
                      />
                    ) : (
                    ...
                    {isEditing ? (
                      <Select
                        ...
                        styles={{
                          control: (baseStyles, state) => ({
                            ...baseStyles,
                            boxShadow: state.isFocused ? '0 0 0 1px grey' : 'none', /*boxShadow: state.isFocused ? '0 0 0 1px grey' : 'none',*/
                            borderRadius: '0',
                            backgroundColor: darkMode ? '#2b2b2b' : 'white', // Utilisation de darkMode
                            color: darkMode ? 'white' : 'black', // Texte adapté au mode
                            border: `1px solid ${darkMode ? '#444' : '#ccc'} !important`,
                        ...
                        value={optionTypeVin.find((option) => option.value === vin?.Type) || ""}
                        isClearable={isClearable}
                        isSearchable={isSearchable}
                        name="Type"
                        placeholder="Type"
                        ...
                        onChange={(selectedOption) => {
                          handleInputChange(selectedOption?.value || '', 'Type');
                          handleInputChange(selectedOption?.label || '', 'Type');
                        }} />
                    ) : (
                    ...
                    {isEditing ? (
                      <>
                        <div className="flex items-center gap-4">
                          ...
                          />
                          <span className="text-lg font-semibold w-12 text-center dark:text-white">{vin.Note_sur_20}</span>
                        </div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">{vin.Note_sur_20 || '-'} ({getNoteDescription(vin.Note_sur_20)})</p>
                    )}
                  </div>
                              ...
                              <div className="flex flex-wrap gap-1">
                                {associationsMets.map((met, index) => (
                                  <button
                                    ...
                                    className={`inline-flex items-center px-2 py-1 rounded-full text-xs border transition-colors cursor-pointer ${
