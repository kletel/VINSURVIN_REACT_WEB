...
if (
  isset($_POST['actilangue_quote_nonce']) &&
  wp_verify_nonce($_POST['actilangue_quote_nonce'], basename(__FILE__)) &&
  pdscaptcha($_POST)
) {
		...
		$m_meta_current_coursePrice = ((double)$quote_current_coursePrice);

		$m_val_prom_percent = ((double)$quote_current_SimuCodePromo / 100);
...
$m_meta_current_coursePrice = ((double)$data_form['current_coursePrice']);

$m_val_prom_percent = ((double)$data_current_SimuCodePromo / 100);
$m_val_prom_price = $m_meta_current_coursePrice * $m_val_prom_percent;
$m_finale_price_promo = $m_meta_current_coursePrice - $m_val_prom_price;
	...
	*/
	...
	if(username_exists($client_email) == false){
        ...
        $user = new WP_User($id_eleve);
        if (empty($user->roles)) {
            $user->set_role('subscriber');
        }
        update_user_meta($id_eleve, 'aiowps_account_status', 'pending');
        do_action('user_register', $id_eleve);
...
$SimuDate  = $_POST['SimuDate']  ?? $_POST['current_SimuDate'] ?? '';
$SimuCours = $_POST['SimuCours'] ?? $_POST['id_current_course_ACTI'] ?? '';
$SimuDuree = $_POST['SimuDuree'] ?? $_POST['current_courseDuration'] ?? '';
$SimuLogement = $_POST['SimuLogement'] ?? $_POST['current_Logement'] ?? '';

$CalaDateArrivee = $_POST['CalaDateArrivee'] ?? '';
$SimuCodePromo = $_POST['code-promo-acti'] ?? '';

$delf = (int)($_POST['delf-acti'] ?? 0);   // 0 = non, 1 = oui
$SimuDELF_txt = ($delf === 1) ? 'Vrai' : 'Faux';  // pour l'API
$SimuDELF = $delf;                                // pour la session
...
/*******************
*
* Get Json (WP HTTP API)
*
*/

$error = null;

// Si l'API attend Vrai/Faux, remplace $SimuDELF par $SimuDELF_txt ci-dessous
$url = 'https://sgbd.kletel.net/4DAction/ActiURL?' . http_build_query([
  'SimuDate'      => $SimuDate,
  'SimuCours'     => strtoupper($SimuCours),
  'SimuDuree'     => $SimuDuree,
  'SimuLogement'  => $SimuLogement,
  'SimuLangue'    => $SimuLangue,
  'SimuCodePromo' => $SimuCodePromo,
  'SimuDELF' => $SimuDELF_txt,
]);
...
if (is_wp_error($res)) {
  ...
  $json_ws = wp_remote_retrieve_body($res);
  $obj_ws  = json_decode($json_ws);

  // si JSON invalide / réponse inattendue -> on évite un fatal plus bas
  if (!is_object($obj_ws) || !isset($obj_ws->V_Prix)) {
...
$prix_str = preg_replace('/[^\d,\.]/', '', (string)$obj_ws->V_Prix);
$val_prix = (float) str_replace(',', '.', $prix_str);
											...
											<div class="niv_acti">
												<div class="in_niv_acti">
													...
													<div class="titrea_popop">
														<div class="nombre_cours">1</div> <div class="vr_titrea_popop"><?php echo __("Quel est votre niveau ?", "acti"); ?></div>
													</div>												
...
function acti_optiona_func()
{
    ...
    add_submenu_page( 'actilangue-options', 'Modules de paiement', 'Modules de paiement',
    'manage_options', 'actilangue-modules-paiement-options', 'actilangue_modules_paiement_options_func');
}
...
$val_prom_percent = ((double)$val_promotion / 100);
...
add_action('pre_user_query', function (WP_User_Query $q) {
    // On ne corrige que dans l'admin, sur les pages AIOS
    if (!is_admin()) return;

    $is_aios_screen = isset($_GET['page']) && strpos($_GET['page'], 'aiowpsec') === 0;
    if (!$is_aios_screen) return;

    if (strpos($q->query_where, 'WHERE = AND') !== false) {
        $q->query_where = preg_replace('/WHERE\s*=\s*AND/i', 'WHERE 1=1 AND', $q->query_where);
    }

    global $wpdb;
    $needle = $wpdb->users . '.ID<>';
    if (strpos($q->query_where, $needle) !== false && strpos($q->query_where, $needle.'0') === false) {
        $q->query_where = str_replace($needle, $needle.'0', $q->query_where);
    }
}, 999);